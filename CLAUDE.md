...
| Hero tile order | Swap the homepage hero tiles to **Industries → Solutions → Products** (was Industries → Products → Solutions, the 2026-07-17 order). Flagged 2026-07-27, **not yet implemented** — see `TASKS.md` Task 11. |
| Products catalog — UCS Cloud | Add a "UCS Cloud" tile to `/products/`, linking through to Solutions — the cloud platform deserves its own catalog entry, not just an implied part of the hardware list. Flagged 2026-07-27, **not yet implemented** — see `TASKS.md` Task 12. |
| Hosting | Not Vercel for the production site — real infra goes through the company's existing `terraform-cloud` repo (Terraform Cloud–managed, AWS account `008568556096`), matching the pattern already used for `rma.utilcell.com`/Terrasense/Ucsnose. S3 + CloudFront (OAC), CloudFront default domain only for now — sensweight.com currently redirects to a live site at `en.sensweight.eu`, so DNS cutover is a deliberately separate later step. Decided + built 2026-07-30, PR `utilcellsro/terraform-cloud#115` (not yet merged/applied) — see `TASKS.md` Task 13. |
| Dealer form backend | Real backend via **Lambda + API Gateway triggering AWS SES** — sends a confirmation email to the submitting client and a notification email to the sales team, replacing the current `mailto:` fallback. Decided 2026-07-27, **built 2026-07-30** (uses the already-verified `unifiedcloudsensors.com` SES identity, no new domain verification needed) — infra in the same PR as Hosting above, `dealer-form.js` already POSTs to `/api/dealer-request` on `main`. Still pending: TFC apply, live end-to-end test, and a real (non-placeholder) sales inbox — see `TASKS.md` Task 13. |
...
- [ ] Real dealer-form backend — **decided 2026-07-27: Lambda + API Gateway triggering AWS SES**, emailing both the submitting client (confirmation) and the sales team (notification), replacing the current `mailto:` fallback at the bottom of `/how-to-buy/`. Not yet built — see `TASKS.md` Task 13.
- [ ] Confirm the sales/dealer inbox that SES should notify (still open — same placeholder address as before)
