...
| unifiedcloudsensors.com — content scope | **Confirmed 2026-09-01:** company story/about (patent EP 4 524 526, TRL 9, Ostrovačice HQ — same sourced facts as sensweight's `/about/`, reframed corporate-tone), Team/Leadership, Press/News, Careers, plus Contact. No ROI calculator, no demo, no per-industry content — those stay sensweight-only. Superseded in part 2026-09-03: Solutions is now a real section on this page too (routing cards/tiles to sensweight.com, not duplicated depth) — see the pivot entry below. |
| unifiedcloudsensors.com — procimo-minimal pivot (2026-09-03) | Boss feedback relayed by user rejected the dark-navy-glow direction: wanted something closer to procimo.com — minimal, restrained, real photography over glow/glass panels, header decluttered, more "life." Reworked in one session, v3→v9 on the same canvas: near-black ground (not navy-tinted) with a light variant added alongside it as a second artboard on the same canvas (not a separate draft); display font swapped from Chakra Petch to **Bricolage Grotesque** for this domain only (sensweight.com keeps Chakra Petch — this is the one deliberate non-literal-DNA font exception, scoped to unifiedcloudsensors.com); Values icon-grid replaced with a plain numbered list; Solutions rebuilt as large photographic tiles ("Featured Projects" style) linking to sensweight.com's real Solution pages, now positioned before Story (user's explicit order); Updates carousel made genuinely live (auto-advances every 5s, pauses on hover); header nav cut from 8 links to 3 + a hamburger, matching procimo's exact restraint. Real photography: no clean UCS-owned photos exist yet, so 5 free/licensed Unsplash photos (Unsplash License, commercial use, no attribution required) were sourced and duotone-toned to the site's palette as an honest stand-in — explicitly not claimed as real UCS installations, swap for real site/product photography later. Canvas unchanged: `https://claude.ai/code/artifact/d31afa1b-4220-4994-ae8e-826faf2203e7`. Sent for review 2026-09-03 as `references/ucs-web_v4.zip` (dark `index.html` + light `light.html`, both reordered). See `unifiedcloudsensors/TASKS.md` for the full ticket-by-ticket history. |
...
Small, separate, minimal/premium corporate "about us" site — distinct from sensweight.com's product/industry sales content, safe to link from LinkedIn. Currently just a disposable placeholder (`unifiedcloudsensors/README.md` + a plain static `index.html`, no build step, no design system). Not yet built as real code. Canvas: `https://claude.ai/code/artifact/d31afa1b-4220-4994-ae8e-826faf2203e7` (now two artboards side by side — Dark and Light, same content; re-read it before continuing design work rather than assuming the state described here is current — the user may have edited it directly, or feedback from a review round may not be reflected here yet).

**Version/review history:** v1 sent for review 2026-09-01. v2 (2026-09-02, added Values/registration number/news items) was never sent out on its own. **2026-09-03: boss feedback on v2 arrived indirectly (relayed by user) and drove a same-day procimo-minimal redesign pivot** (see the "procimo-minimal pivot" decision row above for what changed — colors/typography/real photography/Solutions restructure/nav/light variant/section order). That pivot iterated through several rounds on the canvas (v3–v9 internally) and was **sent for review 2026-09-03 as `references/ucs-web_v4.zip`** (contains both the dark `index.html` and the new light `light.html`, both with the corrected section order — Solutions before Story). **Next step: wait for that review's feedback, then fold it in — not a rebuild.**

A static, standalone snapshot (no editor chrome, just the plain rendered page) is kept at `references/unifiedcloudsensors-design-draft/` — `index.html` (dark) and `light.html` (light), both regenerated to match the canvas each time it's zipped for review. The snapshot is a point-in-time export, not synced automatically when the canvas changes — regenerate both files before zipping if the canvas has moved on. Earlier review-round zips (`ucs-web_v1.zip`, `ucs-web_v3.zip`) are kept in `references/` for history.
...
**2026-09-02 update — v2: three gaps filled in on top of v1, sourced from analyzing the legacy `unifiedcloudsensors.eu` site (not from summarizing it, per the DNA-sourcing rule above):** compared the canvas against `.eu`'s live content; most of `.eu` (Digitalization Guide, Products, Solutions, FAQ, per-industry pages, CZ/PL/EN switcher, catalog PDF) is deliberately out of scope per the content-scope decision above, but three things filled a real gap without violating it — all shipped straight to the canvas (no separate sign-off round, since these are additive facts, not a direction change):
- **Values section** ("02 — What We Stand For", between the Story stat wall and Leadership) — Savings / Professionalism / Trust / Long-Term Service, translated from `.eu`'s four core values (Úspora/Profesionalita/Důvěra/Dlouhodobý servis), with matching inline-SVG icons. Sections renumbered 01–05 (Story, Values, Leadership, Updates, Careers); nav + footer links updated to include Values.
- **Company registration number** (IČ 23272686) added to the Contact/footer block next to the address — was missing entirely before.
- **3 real dated news items** added to Updates, above the existing 3 undated LinkedIn taglines: PF 2026 (18.12.2025), UCS at DTX Europe 2025 (14.9.2025), SYMAS 2024 invitation (7.10.2024) — gives the section actual timeline substance instead of only marketing quotes.

The static snapshot (`references/unifiedcloudsensors-design-draft/index.html`) was regenerated to match, and this batch is what the user has designated **v2**. **v2 has not gone to the boss/chief yet** — the 2026-09-01 review round already in flight with them is v1, pre-addition.
