...
| Repo ownership | Move the repo from the personal GitHub account (`echetvergov/ucs-frontend`) into the **`utilcellsro`** organization, as its own separate repo. Flagged 2026-07-27, **not yet executed** — hard-to-reverse infra action, needs scope confirmation first (transfer vs. fresh repo, rename, redeploy hooks) — see `TASKS.md` Task 10. |
...
| Dealer form backend | Real backend via **Lambda + API Gateway triggering AWS SES** — sends a confirmation email to the submitting client and a notification email to the sales team, replacing the current `mailto:` fallback. Decided 2026-07-27 (resolves the long-open Phase 5 item), **not yet built** — see `TASKS.md` Task 13. |
